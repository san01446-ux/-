...
def register_v1720_living_world_bonds(
    ...
    async def world_broadcast_setting(ctx: commands.Context, mode: str = "상태") -> None:
        ...
        await ctx.send(_t(locale,f"📡 방송 채널: {f'<#{channel_id}>' if channel_id else '설정 안 됨'} · `!세계방송설정 켜기/끄기`",f"📡 Broadcast channel: {f'<#{channel_id}>' if channel_id else 'not set'} · `!worldbroadcast on/off`"))
