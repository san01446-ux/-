...
BASE_QUESTIONS = [
    ...
    {"category":"아포칼립스","q":"월드보스전에서 가장 높은 보상을 노리려면 무엇이 중요할까요?","choices":["꾸준히 공격해 누적 피해를 높인다","한 번도 참여하지 않는다","채팅만 한다","장비를 모두 해제한다"],"answer":"1","aliases":["누적 피해","꾸준히 공격"]},
    {"category":"아포칼립스","q":"감염도가 높아졌을 때 가장 먼저 확인할 시설은?","choices":["병원","시장","카지노","경기장"],"answer":"1","aliases":["병원"]},
]
...
def register_quiz_commands(bot, get_user, check_registered, save_data, world_data, send_pages, add_season_points=None):
    def get_pool():
        ...
        return BASE_QUESTIONS + custom

    def today_info(guild_id):
        today = datetime.now().strftime("%Y-%m-%d")
        pool = get_pool()
        index = _seed(f"quiz:{guild_id}:{today}") % len(pool)
        question = pool[index]
        golden = (_seed(f"golden:{guild_id}:{today}") % 100) < 3
        return today, index, question, golden

    @bot.command(name="오늘의퀴즈", aliases=["일일퀴즈"])
    async def daily_quiz(ctx):
        if not await check_registered(ctx):
            return
        today, index, q, golden = today_info(ctx.guild.id if ctx.guild else 0)
    ...
    async def answer_quiz(ctx, *, 답안: str):
        ...
        accepted = [q["answer"]] + q.get("aliases", [])
        correct = _clean(답안) in {_clean(x) for x in accepted}
        if not correct:
            remain = 3 - quiz_state["attempts"]
            save_data()
            await ctx.send(f"❌ 오답입니다. 남은 기회: **{remain}회**")
            return
    ...
    async def quiz_ranking(ctx):
        ...
        if not rankings:
            await ctx.send("📭 아직 오늘의 퀴즈 정답자가 없습니다.")
            return
    ...
    async def quiz_add(ctx, *, 내용: str):
        ...
        if len(parts) != 6 or parts[5] not in {"1","2","3","4"}:
            ...
            return
        custom = world_data.setdefault("custom_quizzes", [])
        custom.append({"category":"서버퀴즈","q":parts[0],"choices":parts[1:5],"answer":parts[5],"aliases":[parts[int(parts[5])]]})
        save_data()
