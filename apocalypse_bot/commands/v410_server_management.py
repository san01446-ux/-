...
def register_v410_server_management(
    ...
    async def management_help(ctx: commands.Context) -> None:
        if not await require_operator(ctx):
            return
        embed = discord.Embed(
            title="🛡 ABADDON SERVER GUARD V4.1",
        ...
        embed.add_field(
            name="🤖 자동 관리",
            value=(
                "`!자동관리 켜기/끄기` · `!초대차단 켜기/끄기`\n"
                "`!욕설차단 켜기/끄기` · `!욕설추가 단어` · `!욕설삭제 단어`\n"
                "`!예외채널 #채널` · `!예외채널해제 #채널` · `!자동처벌 켜기/끄기`"
            ),
            inline=False,
        )
        embed.add_field(
    ...
    async def toggle_automod(ctx: commands.Context, 상태: str) -> None:
        ...
        if enabled is None:
            await ctx.send("⚠️ 사용법: `!자동관리 켜기` 또는 `!자동관리 끄기`")
            return
    ...
    async def toggle_invites(ctx: commands.Context, 상태: str) -> None:
        ...
        enabled = parse_toggle(상태)
        if enabled is None:
            await ctx.send("⚠️ 사용법: `!초대차단 켜기` 또는 `!초대차단 끄기`")
            return
    ...
    async def toggle_bad_words(ctx: commands.Context, 상태: str) -> None:
        ...
        enabled = parse_toggle(상태)
        if enabled is None:
            await ctx.send("⚠️ 사용법: `!욕설차단 켜기` 또는 `!욕설차단 끄기`")
            return
    ...
    async def toggle_auto_timeout(ctx: commands.Context, 상태: str) -> None:
        ...
        enabled = parse_toggle(상태)
        if enabled is None:
            await ctx.send("⚠️ 사용법: `!자동처벌 켜기` 또는 `!자동처벌 끄기`")
            return
