...
def _validate_bet(amount: int) -> Optional[str]:
    ...
    if amount > MAX_BET:
        return f"최대 참가비는 **{MAX_BET:,}칩**입니다."
    return None
...
class CardLobbyView(discord.ui.View):
    ...
    async def join(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        async with self.lock:
            ...
            if casino_chips(user) < self.bet:
                await interaction.response.send_message(f"참가비가 부족합니다. 현재 **{casino_chips(user):,}칩**", ephemeral=True)
                return
    ...
    async def start_game(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        async with self.lock:
            ...
            for uid, name in self.players.items():
                if casino_chips(self.get_user(uid)) < self.bet:
                    insufficient.append(name)
            if insufficient:
                await interaction.response.send_message("참가비가 부족한 생존자: " + ", ".join(insufficient), ephemeral=True)
                return
            self.started = True
            await interaction.response.defer()
    ...
    def _reserve(self) -> None:
        ...
        try:
            for uid in self.player_ids:
                user = self.get_user(uid)
                if casino_chips(user) < self.bet:
                    raise ValueError(f"{self.names[uid]} 참가비 부족")
...
def register_v651_card_games(
    ...
    async def create_lobby_from_interaction(interaction: discord.Interaction, kind: str, bet: int) -> Tuple[bool, str]:
        ...
        if casino_chips(get_user(int(interaction.user.id))) < bet:
            return False, f"참가비가 부족합니다. 현재 **{casino_chips(get_user(int(interaction.user.id))):,}칩**"
    ...
    async def create_lobby_from_ctx(ctx: commands.Context, kind: str, bet: int) -> None:
        ...
        if casino_chips(user) < bet:
            await ctx.send(f"참가비가 부족합니다. 현재 **{casino_chips(user):,}칩**")
            return
        min_players, max_players = (2, 8) if kind == "조커잡기" else (2, 6)
        factory_map = {"포커": PokerSession, "원카드": OneCardSession, "조커잡기": JokerSession}
        lobby = CardLobbyView(
