...
def register_v1094_image_table_patch(
    ...
    @bot.command(name="이미지검수", aliases=["imageaudit", "renderaudit", "cardimageaudit"], help="v10.9.4 PNG 글꼴·레이아웃·카드 테이블 연결을 검사합니다.")
    ...
    if patch_notes is not None:
        async def v1094_patch_notes(ctx: commands.Context) -> None:
            ...
            embed.add_field(name=_t(locale, "🧪 최신 검수", "🧪 Latest Audit"), value=_t(locale, "`!테스트 상세`와 `!이미지검수 상세`가 v10.9.4에서 수정한 경로만 검사합니다.", "`!test detail` and `!imageaudit detail` check only v10.9.4 paths."), inline=False)
            await ctx.send(embed=embed)
