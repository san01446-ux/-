...
def register_v32_commands(
    ...
    async def server_settings(ctx):
        ...
        await ctx.send(
            ...
            "관리자 설정 명령어\n"
            "`!서버채널 공지` · `!서버채널 RPG` · `!서버채널 퀴즈`\n"
            "`!서버기능 도감 켜기/끄기` · `!서버기능 튜토리얼 켜기/끄기`"
        )
    ...
    async def server_channel(ctx, 종류: str):
        ...
        if key in ("공지", "announcement"):
            ...
            quiz = world_data.setdefault("quiz_notifications", {}).setdefault(str(ctx.guild.id), {})
            quiz.update({"enabled": True, "channel_id": ctx.channel.id})
            text = "퀴즈 자동 알림"
        else:
            await ctx.send("⚠️ 사용법: `!서버채널 공지/RPG/퀴즈`")
            return
        save_data()
    ...
    async def server_feature(ctx, 기능: str, 상태: str):
        if not admin_only(ctx):
            await ctx.send("❌ 서버 관리자만 설정할 수 있습니다.")
            return
        mapping = {"도감": "codex_notifications", "튜토리얼": "tutorial_notifications"}
        feature_key = mapping.get(기능)
        if not feature_key or 상태 not in ("켜기", "끄기", "on", "off"):
            await ctx.send("⚠️ 사용법: `!서버기능 도감/튜토리얼 켜기/끄기`")
            return
        enabled = 상태 in ("켜기", "on")
        guild_settings(ctx.guild.id)[feature_key] = enabled
        save_data()
        await ctx.send(f"✅ **{기능} 알림**을 {'켰습니다' if enabled else '껐습니다'}.")
