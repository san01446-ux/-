...
def register_v1000_global_survivor(
    ...
    async def getting_started(ctx: commands.Context) -> None:
        locale = _locale_from_context(ctx)
        if locale == "ko":
            lines = ["1. `!가입 생존자`", "2. `!직업목록` → `!직업선택 직업명`", "3. `!파밍출발 마트`", "4. `!할일`", "5. `!생존도감`"]
        else:
            lines = ["1. `!register survivor`", "2. `!jobs` → `!choosejob job`", "3. `!startexpedition market`", "4. `!tasks`", "5. `!survivalcodex`"]
        await ctx.send(embed=discord.Embed(title=L(locale, "getting_started_title"), description="\n".join(lines), colour=0x2ECC71))
    ...
    async def returning_guide(ctx: commands.Context) -> None:
        ...
        profile["last_seen_version"] = VERSION
        save_data()
        if locale == "ko":
            lines = [f"마지막 확인 버전: **v{previous}**", "🌐 완전 한·영 화면 분리", "📋 임무 추적기", "📚 통합 생존 도감", "🤝 NPC 인연 기억", "🛰️ 주간 글로벌 탐사", "🔔 알림·보상 회수 센터"]
        else:
            lines = [f"Last reviewed version: **v{previous}**", "🌐 Fully separated Korean/English interface", "📋 Mission tracker", "📚 Integrated survival codex", "🤝 NPC relationship memory", "🛰️ Weekly global expedition", "🔔 Notification and reward center"]
        await ctx.send(embed=discord.Embed(title=L(locale, "returning_title"), description="\n".join(lines), colour=0xE67E22))

    @bot.command(name="인연", aliases=["relationships", "bonds"], help="주요 NPC가 기억하는 개인 관계 기록을 확인합니다.")
