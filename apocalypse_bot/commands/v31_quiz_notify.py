...
from apocalypse_bot.commands.daily_quiz import BASE_QUESTIONS
...
def _today_question(guild_id: int, date_text: str):
    index = _seed(f"quiz:{guild_id}:{date_text}") % len(BASE_QUESTIONS)
    question = BASE_QUESTIONS[index]
...
def register_v31_commands(bot, get_user, check_registered, save_data, world_data):
    ...
    async def post_start(guild, channel, state, date_text):
        q, golden = _today_question(guild.id, date_text)
    ...
    async def post_end(guild, channel, state, date_text):
        q, _ = _today_question(guild.id, date_text)
