...
def _checks(bot: commands.Bot) -> List[Tuple[str, bool, str]]:
    ...
    return [
        ...
        ("상태·시간 선택", source_flags["mode_select"], "켜기/끄기/항상/09~23/08~22"),
...
def register_v1151_alert_settings_ui(
    ...
    if patch_notes is not None:
        async def v1151_notes(ctx: commands.Context) -> None:
            ...
            embed.add_field(name=_t(locale, "드롭다운 설정", "Dropdown setup"), value=_t(locale, "알림 종류·켜기/끄기·채널·멘션 역할을 한 화면에서 선택합니다.", "Choose alert type, on/off state, channel and mention role in one panel."), inline=False)
