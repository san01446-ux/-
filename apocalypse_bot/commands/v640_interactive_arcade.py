...
def register_v640_interactive_arcade(
    ...
    async def minigame_list(ctx: commands.Context):
        embed = discord.Embed(title="🕹️ ABADDON 미니게임 센터", description="결과와 손익을 명확히 표시하고, 버튼·이모지 중심으로 쉽게 참여할 수 있습니다.", color=discord.Color.dark_teal())
    ...
    if test is not None:
        async def v640_test(ctx: commands.Context, 모드: str = "기본"):
            ...
            dup = {k: v for k, v in tokens.items() if len(set(v)) > 1}
            ...
            guide_missing = [name for name in expected[:12] if f"!{name}" not in guide_text]
            ...
            checks.append(("지뢰 손익 문구", all(word in __import__('inspect').getsource(__import__('apocalypse_bot.commands.v638_hardcore_arcade', fromlist=['MinesView']).MinesView) for word in ("시작 전 보유", "얻은 돈", "잃은 돈")), "총액·순손익·현재 보유 분리"))
