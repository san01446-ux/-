...
async def perform_life_activity(ctx, activity):
    ...
    amount = max(1, int(random.randint(minimum, maximum) * exploration_modifier(u)))
...
async def 거래소(ctx):
    ...
    for listing_id, listing in sorted(
        ...
        lines.append(
            ...
            f"| {listing['price']:,}개 | 판매자 <@{listing['seller']}>"
        )
